...
Se eliminó la creación y reconversión de tensores temporales por movimiento y estado volátil. En los dos frames originales se conservan exactamente decisiones y valores, y las latencias locales bajan aproximadamente 25%. No se cambian el árbol ni sus reglas; falta confirmar el ahorro y el límite de 30 s en RTX. [Evidencia](direct-encoding-results.json) y [comando vigente](../RECUPERAR_EVALUACION.md).
...
La integración acumulada suma 43 partidas. Las dos decisiones reproducidas tardaron 16.26 s y 4.79 s en CPU/macOS. Esto no valida el rendimiento en Windows/RTX ni demuestra mejora de juego. Sigue pendiente ejecutar el procedimiento CPU/CUDA y la comparación de 400 partidas de [RECUPERAR_EVALUACION.md](../RECUPERAR_EVALUACION.md). [Evidencia verificable](performance-results.json). Las secciones inferiores conservan el historial de cada tanda y sus conteos de entonces.

## Actualización operativa: timeout en NVIDIA

El reporte de la RTX 4090 supera el límite anterior de 30 s. Se añadió `--decision-seconds` (default 30; piloto explícito 60), con validación, metadatos y diagnóstico. No se podó búsqueda ni cambió el contrato de las experiencias exitosas. Pasaron **575 pruebas**; una partida adicional con semilla 7331 y pesos congelados terminó en CPU en 25 turnos, con 61 solicitudes auditadas sin discrepancias. No se reprodujo de manera independiente la medición del frame de la RTX 4090: falta repetir allí con 60 s. El gate incorpora únicamente el nuevo hash del entrenador; los demás hashes permanecen intactos. [Instrucciones](../TIMEOUT_GPU.md) y [evidencia](timeout-results.json).
